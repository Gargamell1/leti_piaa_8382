...
void Graph::add_edge(int v1, int v2, int cap)
{
    edge_id_t
...
void Graph::mod_edge(int v, edge_id_t e, int dcap)
{
    Edge
        & e1 = edge(v, e),
        & e2 = revedge(v, e);
...
int Graph::path_flux(const Path& p) const
{
    int f = INT_MAX;
...
void Graph::apply_flux(const Path& p, int f)
{
    int v = start;
    auto iter = p.begin();
    for (; v != end; v = edge(v, *iter++).dest)
...
std::ostream& debug_write_path(const Graph& g, std::ostream& os, const Path& p)
{
    int v = g.start;
    auto iter = p.begin();
...
Path Graph::recover_path(const std::vector<edge_id_t>& revs) const
{
    Path p{};
...
int read_vertex(std::istream& is, Graph& g)
{
    char c;
    is >> c;
...
std::ostream& write_edge(std::ostream& os, const Graph& g, int v, edge_id_t e)
{
...
std::ostream& write_vertex_edges(std::ostream& os, const Graph& g, int v)
{
    struct EdgeRef {
        int dest;
        edge_id_t ei;
...
std::ostream& write_flux(std::ostream& os, const Graph& g)
{
